...
export default function StayApplyButton({
  ...
  const [timeLeft, setTimeLeft] = useState<string>(""); // Fetch user's booking status for this stay
  ...
  }, [stayId, session, sessionStatus]); // Countdown timer for pending payments
  ...
  }, [bookingStatus]); // Check if slots are available

  const noSlotsAvailable = slotsAvailable === 0; // Loading state
...
"
  ...
  if (bookingStatus?.hasBooking) {
    const { status, bookingId, expiresAt, canPay } = bookingStatus; // CONFIRMED
      ...
      return (
        <div className={className}>
          ...
          {noSlotsAvailable ? (
            <button
              disabled
              className="w-full bg-white text-[#172a46]text-xl font-bold py-5 px-12 rounded-full inline-flex items-center justify-center gap-3 opacity-50 cursor-not-allowed"
            >
              <span>Sold Out</span>
            </button>
          ) : (
    ...
    if (status === "FAILED" || status === "EXPIRED") {
      return (
        <div className={className}>
          <div className="text-center mb-3">
            <p className="text-[#E7E4DF] text-lg font-semibold">
              Payment Failed
            </p>
          </div>
  ...
  return (
    <div className={className}>
      {slotsAvailable <= 3 && (
        <div className="text-center mb-3">
          <p className="text-[#E7E4DF] text-sm font-semibold animate-pulse">
